  ...
  async function loadImages() {
    ...
    try {
      ...
      for (let index = 0; index < responseJson.wallpapers.length; index++) {
        ...
        newElements.push(masonry_item); // Append items to the list
      }
  
      newElements.forEach(item => { item.style.opacity = "0"; });
  
      // ✅ Pre-calculate item height from width/height before layout
  
  
      // Append new elements to Masonry
      newElements.forEach(el => wallpaper_container.appendChild(el));

      // 🔥 Trigger a full relayout (NOT just appended)
      // masonry.reloadItems();
